...
### Phase 3 Board Integration — IMPLEMENTED + VERIFIED IN SIMULATION (2026-08-21)

The board-facing AXI4-Lite architecture (Decisions 17–20) is implemented and
verified in simulation; only physical measurement and bitstream generation
remain BOARD-ONLY. See `docs/PHASE3_BOARD_INTEGRATION.md` and
`docs/PHASE3_SYNTHESIS.md`.

- **New RTL:** `rtl/common/cnn_axi_ctrl.sv` (AXI4-Lite slave + result BRAM),
  `rtl/top/cnn_top.sv` (reset conversion + top).
- **New testbench:** `sim/tb_cnn_top_axi.sv` — 17/17 PASS (bit-exact vs golden).
- **New software:** `software/cnn_host.c` (PS UIO driver/benchmark), Makefile.
- **New scripts:** `scripts/run_verification.sh`, `scripts/board/*`
  (overlay + block-design template + firmware shell).
- **Reproducibility:** `golden_canonical.hex` now generated by
  `python/reference/export_vectors.py`; `data/vectors/SHA256SUMS` added;
  manifest `git_sha` re-pinned; `LICENSE` (MIT) added.
- **No frozen module modified.** `cnn_accelerator_v2.sv` gained one
  `sparsity_disable` input (non-frozen integration logic).
...
### Decision 17 — Measured WS-dense via `sparsity_disable` — RESOLVED (2026-08-21)

**A `sparsity_disable` control input is added to `cnn_accelerator_v2` (Phase-2
integration logic — not frozen), so the Weight-Stationary no-skip cycle bound is
MEASURED in simulation rather than asserted analytically.** Default 0 preserves
the coarse zero-group skip; `sparsity_disable=1` forces WS to run all 896 groups.

| Parameter | Value |
|---|---|
| WS dense (sparsity_disable=1) | **38,528 cycles, bit-exact [SIMULATION]** |
| WS sparse (sparsity_disable=0) | 18,368 cycles [SIMULATION] |
| OS dense | 9,184 cycles [SIMULATION] |
| Effect on the frozen datapath | none (2-input AND on the `ws_skip` predicate) |

This converts the paper's central WS-dense row from `[ANALYTICAL] no-skip bound`
to `[SIMULATION] sparsity_disable=1`, closing the "WS-dense is analytical not
measured" reproducibility gap. `benchmark_matrix.json` provenance updated.

### Decision 18 — A1 fair-WS ablation resolved by analysis — RESOLVED (2026-08-21)

**The `A1` "pipelined-V2" ablation (`RESEARCH_READINESS_PLAN.md` §1/§11) is
resolved by analysis, not implementation.** The Phase-2 WS bubble analysis
(`PHASE2_HARDENING.md` §3) shows the only recoverable bubble is group-boundary
pipelining = 1 cycle/group ≈ 2.3%, which cannot change the 4.195× OS-vs-WS
conclusion (whose mechanism is 8× channel serialization vs ~1.9× shorter group).
Implementing a schedule change for a 2.3% effect is neither technically safe
(risk to bit-exactness) nor scientifically necessary (the conclusion is robust).
The like-for-like comparison is already fair: OS-on-array-v2 is bit-identical to
V1 (`pe_v2.sv` OS mode), on the same 64 DSP48E2 / same clock / same golden.

### Decision 19 — Board-facing AXI4-Lite architecture — RESOLVED (2026-08-21)

**The board-facing architecture wraps the verified accelerator in an AXI4-Lite
slave** (per `RESEARCH_READINESS_PLAN.md` §7 Option A, §8): `cnn_axi_ctrl.sv`
(slave + register file + result BRAM + accelerator instance) and `cnn_top.sv`
(`aresetn`→`rst` conversion). Details in `docs/PHASE3_BOARD_INTEGRATION.md`.
Runtime OS↔WS switching is now reachable by software via `DATAFLOW_MODE` +
`CONTROL.MODE_COMMIT` (previously only by testbench pins). Verified 17/17 PASS
in `sim/tb_cnn_top_axi.sv` (image/weight AXI load, OS/WS/WS-dense cycle counts
9,184/18,368/38,528, 6,272/6,272 bit-exact per mode, runtime switch, counters).

### Decision 20 — Result storage (8-bank canonical BRAM) — RESOLVED (2026-08-21)

**Results are stored in 8 single-write-port BRAMs, one per column residue
`x & 7`, at dense address `flat >> 3`** (`flat = ch*784+y*28+x`). Pixel `c` of a
result word maps to bank `7-c`, dense `(result_base+7-c)>>3`. This gives 8
parallel single-write ports (1 result word/cycle, no FIFO/backpressure) and
re-orders results into canonical order in hardware, so host software reads
straight through with **no reverse-index decode**. The group-leftmost column is
`result_base % 28` (784 = 28·28); pixel validity is `left ≤ 20+c`.

### Decision 21 — License — RESOLVED (2026-08-21, changeable)

**The repository now carries an MIT license** (`LICENSE`), matching the MIT
ONNX-MNIST-12 checkpoint and the norm for academic FPGA code. This is a
low-risk default for publication-ready redistribution; the team may choose a
different license before first public release.
