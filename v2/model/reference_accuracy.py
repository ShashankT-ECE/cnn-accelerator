...
Writes v2/results/reference_accuracy.csv (net, reference_version, precision,
correct, total, accuracy_pct + common.base_meta metadata). Step 2.1c: one row
pair per NET_CONFIGS entry - lenet5_v1, cifar10_r1 (history) and cifar10_r2
(reference of record, DECISIONS D3); checkpoints and npz come from NET_CONFIGS.
...
frozen npz (NET_CONFIGS quant_params) is evaluated in the same loop and must give
the identical correct count.
"""
...
FIELDS = ["net", "reference_version", "precision", "correct", "total", "accuracy_pct"]
CIFAR_KEYS = ("cifar10_r1", "cifar10")          # history first, then reference of record
TEST_N = 10000
CALIB_N = 1024


def _row(key, precision, correct, total, t0) -> dict:
    net = "lenet5" if key == "lenet5" else "cifar10"      # cifar10_r1 is a cifar10 version
    ...
    r.update(net=net, reference_version=ver, precision=precision, correct=int(correct),
             total=int(total), accuracy_pct=f"{100.0 * correct / total:.2f}")
    print(f"  {ver:10s} {precision:5s} {correct}/{total} = {r['accuracy_pct']}%  "
...
def lenet_fp32() -> dict:
    ...
    return _row("lenet5", "FP32", *_fp32(train, preprocess, NET_CONFIGS["lenet5"]["checkpoint"]), t0)


def cifar_fp32(key: str = "cifar10") -> dict:
    from cifar10 import preprocess, train
    t0 = time.perf_counter()
    return _row(key, "FP32", *_fp32(train, preprocess, NET_CONFIGS[key]["checkpoint"]), t0)
...
def lenet_int8() -> dict:
    ...
    model = Int8LeNet5(calibrate(load_weights(REPO_ROOT / NET_CONFIGS["lenet5"]["checkpoint"]), calib))
    correct = 0
...
def cifar_int8_counts(key: str = "cifar10") -> tuple[int, int, float]:
    """(legacy-calibrated correct, frozen-npz correct, t0). Mirrors eval_cifar10_int8.py."""
    from cifar10.int8_model import Int8Cifar10Net
    t0 = time.perf_counter()
    cfg = NET_CONFIGS[key]
    legacy = Int8Cifar10Net(fz.legacy_params(ckpt_rel=cfg["checkpoint"]))
    frozen = Int8Cifar10Net(fz.load_frozen_params(cfg["quant_params"]))
...
def main() -> int:
    rows = [lenet_fp32(), lenet_int8()]
    for key in CIFAR_KEYS:
        rows.append(cifar_fp32(key))
        c_leg, c_frz, t0 = cifar_int8_counts(key)
        rows.append(_row(key, "INT8", c_leg, TEST_N, t0))
        print(f"  {key} INT8 frozen npz: {c_frz}/{TEST_N} (legacy calibrate: {c_leg})")
        if c_frz != c_leg:
            print(f"MISMATCH ({key}): frozen npz != legacy calibrate", file=sys.stderr)
            return 1
